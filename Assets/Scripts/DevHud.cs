...
// V 부피 안개 켜기/끄기 · [ ] 안개 밀도 ÷1.5 ×1.5 · - = 램프 세기 ÷1.25 ×1.25 · 1 2 어둠 적응 환경광 ÷1.25 ×1.25 · 3 4 곡괭이 내구도 −10/+10 · 5 6 스태미나 −20/+20 · 7 8 램프 끄덕임 폭 ÷1.5 ×1.5 (UI-1b 판정용 임시) · 0 괴물 끄기/켜기 (Godot DebugHud 의 0) · F1 표시 끄기
public class DevHud : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (kb.digit7Key.wasPressedThisFrame) lamp.bobDeg /= 1.5f;                                        // 끄덕임 폭 — 사용자가 찾는다 (임시, 확정되면 뺀다)
        if (kb.digit8Key.wasPressedThisFrame) lamp.bobDeg *= 1.5f;
    }
    ...
    void OnGUI()
    {
        ...
        GUI.Label(new Rect(10, 10, 900, 120),
            ...
            $"{player.stance}  stamina {player.stamina:0}{(player.exhausted ? " EXHAUSTED" : "")}  nod x{(player.stamina <= Tuning.STAMINA_SOON ? Tuning.LAMP_BOB_SOON_MUL : 1f):0} LAMP_BOB_DEG {lamp.bobDeg:0.###}   [ 5 6 | 7 8 ]   pick {(pickaxe == null ? "-" : $"{pickaxe.durability:0}/{Tuning.PICK_DURABILITY_MAX:0} {(pickaxe.hasPick ? "held" : pickaxe.Broken ? "BROKEN" : "thrown [E]")}")}   [ 3 4 ]   [F1] hide" + monster);
    }
}
