...
최종 갱신 2026-09-15 (11차). 이 문서 하나만 읽어도 이어서 작업할 수 있어야 한다.
...
| 마일스톤 4 눈·빛·추격·잡기 | `docs/제안서_M4_눈_빛_추격_잡기.md` 구현 → 1차 판정(09-15) 반영(눈 두 개 · 어둠 적응 · 눈 발광) → **빌드·배포물 검사 통과, 재판정 대기 (4절)**. 램프 켜진 몸 12 m·60° → alert 1.0 s → 6.5 m/s 추격 → 1.5 m 잡힘 → 검은 화면 → 3.0 s 뒤 복도 시작점·광석 0. 켜진 램프 30 m 는 2.5 m/s 로 접근. 램프 끄면 3.0 s 뒤 놓친다 |
...
물어볼 것 (마일스톤 4 재판정):
1. 램프 끄고 4 s 뒤 벽·바닥 윤곽이 보이는가 — 어두우면 `14_dark_adapt_16.0.png`(16.0), 밝으면 3.0 캡처와 비교해 값을 고른다
2. 어둠 속 괴물 눈 두 점이 "저기 있다"로 읽히는가, 더 밝아야 하는가
3. 램프 끄고 뒤로 물러나기가 이제 통하는가
4. 30 m 에서 천천히 다가오는 것이 "들켰다"로 읽히는가 (눈이 생겨 방향이 보인다)
