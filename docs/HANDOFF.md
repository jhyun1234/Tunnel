...
최종 갱신 2026-09-15 (10차). 이 문서 하나만 읽어도 이어서 작업할 수 있어야 한다.
...
| 마일스톤 4 눈·빛·추격·잡기 | `docs/제안서_M4_눈_빛_추격_잡기.md` 구현 → **빌드·배포물 검사 통과(09-15), 판정 대기 (4절)**. 램프 켜진 몸 12 m·60° → alert 1.0 s → 6.5 m/s 추격 → 1.5 m 잡힘 → 검은 화면 → 3.0 s 뒤 복도 시작점·광석 0. 켜진 램프 30 m 는 2.5 m/s 로 접근. 램프 끄면 3.0 s 뒤 놓친다 |
...
물어볼 것 (마일스톤 4 눈·빛·추격·잡기, `docs/제안서_M4_눈_빛_추격_잡기.md` 확인 목록). 괴물은 북쪽 끝에서 시작하니 램프 켜고 북쪽으로 걸어가면 30 m 에서 다가오기 시작하고 12 m 에서 멈춰 본다:
1. alert 1.0 s(멈춰서 이쪽을 봄)가 "지금 뛰어야 한다"로 읽히는가, 너무 짧은가 긴가
2. 잡힘 → 검은 화면 → 3 s 재시작이 벌로 느껴지는가, 허무한가
3. 램프 끄고 뒤로 물러나는 것이 통한다고 느껴지는가 (직선이라 숨을 곳은 없다)
4. 램프 켠 채 30 m 에서 천천히(2.5 m/s) 다가오는 것이 "들켰다"로 읽히는가, 그냥 배회로 보이는가
