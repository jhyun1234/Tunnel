...
최종 갱신 2026-09-14 (2차). 이 문서 하나만 읽어도 이어서 작업할 수 있어야 한다.
...
| 마일스톤 1 | 빌드·배포물 검사 통과. **1차 판정 받음(램프 146.8, 안개 0.00089, "+Z 쪽 빛이 안 보임") → 버그 고침 → 재판정 대기** |
| 이동 | `Player.cs` — Godot Player.gd 이동 부분 이식: 걷기 4.5 / 달리기 7 / 숙이기 2, 점프, 스태미나·탈진, 가속 0.12 s·감속 0.10 s |
| 헤드램프 | `Headlamp.cs` — Spot, 반각 60°(Godot 40), 사거리 14, 세기 146.8(사용자), 소프트 섀도, 0.1 s 늦게 따라옴, F 끄기 |
...
| 안개 | 거리 안개 Exponential 0.03 + 부피 안개 패키지 `com.cqf.urpvolumetricfog` 0.5.91(MIT, git) 밀도 0.00089(사용자). PC_Renderer에 렌더러 기능 추가 |
...
1. 마일스톤 1 재판정 (4절). 1차 판정 값은 노멀맵 버그 상태에서 고른 것이다

### B. 내가 할 수 있는 것

1. 재판정 값 → `Tuning.cs` → `MakeScene -force` → `build.sh`
...
물어볼 것:
1. +Z / −Z 양쪽에서 램프 빛이 같게 보이는가
2. 노멀맵이 고쳐져 화면이 밝아졌다(146.8에서 약 5배) — 램프 세기·안개 밀도를 다시 몇으로 두면 좋은가
3. 조작해서 재미있는가 — 걷기·달리기·숙이기 속도, 가속·감속이 무른가
4. 램프 반각 60°가 넓은가, 원형 테두리가 보이는가
5. 부피 안개가 URP를 계속 쓸 만한 수준인가
