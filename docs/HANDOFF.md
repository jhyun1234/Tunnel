...
최종 갱신 2026-09-15 (12차). 이 문서 하나만 읽어도 이어서 작업할 수 있어야 한다.
...
| 마일스톤 4 눈·빛·추격·잡기 | `docs/제안서_M4_눈_빛_추격_잡기.md` 구현 → 1차 판정 반영 → **2차 판정 통과 (09-15)**: 적응 뒤 보인다 · 눈 두 점 읽힌다 · 끄고 물러나기 통한다 · 30 m "들켰나?" → 12 m "들켰다". 남은 것: DARK_ADAPT_AMBIENT 값을 사용자가 `1`/`2` 키로 찾는 중. 램프 켜진 몸 12 m·60° → alert 1.0 s → 6.5 m/s 추격 → 1.5 m 잡힘 → 검은 화면 → 3.0 s 뒤 복도 시작점·광석 0. 켜진 램프 30 m 는 2.5 m/s 로 접근. 램프 끄면 3.0 s 뒤 놓친다 |
...
| 헤드램프 | `Headlamp.cs` — Spot 반각 60°, 사거리 14, 세기 75.2, 가까운 면 감광(광선 9개, REF 4 m · POW 1.4). **어둠 적응**(Godot #32): 끄면 DARK_ADAPT_TIME 4 s 에 걸쳐 환경광 0.03 → DARK_ADAPT_AMBIENT 8.0, 거리 안개 0.03 → DARK_ADAPT_FOG 0.45. 켜면 즉시 평소. `lamp.adapt` 0~1. 곡괭이는 안 비춘다(조명 레이어), 곡괭이 전용 약한 등 `PickLight` 를 같이 켜고 끈다 |
...
1. 마일스톤 2 재판정 (4절)
2. DARK_ADAPT_AMBIENT 확정 — 사용자가 실행 파일에서 `1`(÷1.25)·`2`(×1.25)로 찾아 화면 왼쪽 위 숫자를 알려 주면 `Tuning.cs` 에 박고 검사 문턱(0.010~0.05)을 맞춘다
3. 다음 마일스톤 — 제안서로 하나씩. 순서는 설계서 v2 Step 7(교차 검토 반영): ②c 체력·스턴·철수(캡슐판: 3대 철수·철수 중 무적·램프 원뿔 안 8~14 m 벽 타기·90 s 재등장) → ③ 갱도 조립기(조각 23종) + 감독·NavMesh·배움 카드 → ④ 정비 → ⑤ 나머지 소리
...
M4 2차 판정(09-15): 1. 적응 뒤 보인다 ✓ (값은 `1`/`2` 키로 사용자가 찾는다) 2. 눈 두 점 읽힌다 ✓ 3. 끄고 물러나기 통한다 ✓ — "램프 꺼도 2 m 안이면 잡힘"은 Godot STALKER_FOUND_M(수색 중 2 m)·STALKER_LOSE_S(추격 중 3 s 는 마지막 자리로 계속) 그대로, 의도로 확인 4. 30 m 는 "들켰나?", 12 m 는 "들켰다" ✓ = 설계서 Step 5(빛은 방향만·눈은 확정) 그대로 → **M4 통과**.
